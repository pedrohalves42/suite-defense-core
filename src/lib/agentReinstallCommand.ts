...
export function buildAgentReinstallCommand({
  ...
  const parts = [
    ...
    '  [System.Management.Automation.Language.Parser]::ParseInput($scriptContent, [ref]$parseTokens, [ref]$parseErrors) | Out-Null;',
    "  if ($parseErrors -and $parseErrors.Count -gt 0) { throw ('Script invalido para PowerShell: ' + $parseErrors[0].Message) }",
