...
export function buildAgentReinstallCommand({
  ...
  const parts = [
    ...
    "  if ($content -notmatch 'function\\s+Initialize-Config') { return $false }",
    "  if ($content -notmatch 'function\\s+Main') { return $false }",
    "  if ($content -notmatch 'Start-HeartbeatLoop') { return $false }",
