...
export function buildAgentReinstallCommand({
  ...
  const parts = [
    ...
    '      if ($plain -and $plain.Length -gt 5000) { $scriptContent = $plain; $resolvedBaseUrl = $baseUrl; break }',
    '    } catch {',
    '      $resp = Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop;',
    '      if ($resp.Content -and $resp.Content.Length -gt 5000) { $scriptContent = $resp.Content; $resolvedBaseUrl = $baseUrl; break }',
    '    }',
    '  } catch {',
    ...
    "} else { Write-Host ('Erro: servidor nao retornou script valido. Ultimo erro: ' + $lastErr) -ForegroundColor Red }",
  ];
