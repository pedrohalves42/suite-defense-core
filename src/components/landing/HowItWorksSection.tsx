    ...
    <section className="py-24 relative">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-6xl mx-auto relative">
          <div className="hidden md:block absolute top-16 left-[12.5%] right-[12.5%] h-px bg-gradient-to-r from-info/30 via-cta-positive/30 to-accent/30" />
          <div className="grid md:grid-cols-4 gap-8">
            {howItWorks.steps.map((step, index) => {
              const color = stepColors[index] || stepColors[0];
              return (
                <motion.div key={index} className="relative text-center" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.15 }}>
                  <div className="relative inline-flex mb-6">
                    <div className={`w-14 h-14 ${color.bg} ${color.text} rounded-2xl flex items-center justify-center font-bold text-xl shadow-lg ${color.shadow}`}>{step.number}</div>
