...
/**
 * Component to register v4.1.0 releases with Ed25519 signatures
 * SSA-004: Payload Signing implementation
 */
export function RegisterReleasesV410() {
  ...
  const releaseNotes = `SSA-004: Payload Signing - Ed25519 job signature verification
...
CHANGES:
- Added Ed25519 public key (SPKI format)
- Added verify_job_signature function
- Jobs without valid signatures are rejected
    ...
    try {
      for (const platform of platforms) {
        ...
        try {
          ...
          toast.success(`${platform.name} v4.1.0 registrado com sucesso`);
    ...
    <Card className="border-primary/20">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle>Registrar Releases v4.1.0 (SSA-004)</CardTitle>
        </div>
        <CardDescription>
          Payload Signing - Ed25519 job signature verification para Windows, Linux e macOS
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="rounded-lg bg-muted/50 p-4 text-sm">
          <h4 className="font-semibold mb-2">Novidades v4.1.0:</h4>
          <ul className="list-disc list-inside space-y-1 text-muted-foreground">
            <li>Verificação de assinatura Ed25519 em payloads de jobs</li>
            <li>Rejeição de jobs sem assinatura válida</li>
            <li>Proteção contra RCE via banco comprometido</li>
            <li>Chave pública SPKI embarcada nos agentes</li>
          </ul>
        </div>
