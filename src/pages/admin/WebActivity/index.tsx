...
export default function WebActivity() {
  ...
  const handleCollectAllWebActivity = useCallback(async () => {
    ...
    try {
      ...
      await (supabase as never).from('jobs').update({ status: 'cancelled', updated_at: new Date().toISOString() }).eq('tenant_id', tenantId).eq('type', 'collect_web_activity').in('status', ['queued', 'pending']).in('agent_id', agentIds);
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { error: insertError } = await (supabase as never).from('jobs').insert(jobs);
