  ...
  const { data: entries, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select('id, original_job_id, tenant_id, agent_name, agent_id, job_type, error_message, error_count, failure_class, classification, status, first_failure_at, last_failure_at, next_retry_at, max_retries, flagged_suspicious, auto_flagged_reason, metadata, created_at, decision_event_id')
