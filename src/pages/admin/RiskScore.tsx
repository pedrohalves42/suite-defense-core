...
const BREAKDOWN_CONFIG: Record<keyof RiskBreakdown, { 
  ...
  antivirus_issues: {
    icon: ShieldAlert,
    label: 'Antivírus',
    description: 'Computadores com antivírus desativado ou desatualizado',
    link: '/admin/antivirus-status',
  },
  critical_vulnerabilities: {
    icon: Bug,
    label: 'Vulnerabilidades',
    description: 'Vulnerabilidades críticas encontradas no sistema',
    link: '/admin/vulnerabilities',
  },
  offline_agents: {
    icon: WifiOff,
    label: 'Computadores Offline',
    description: 'Computadores que não estão respondendo',
    link: '/admin/agent-health',
  },
  critical_events: {
    icon: AlertTriangle,
    label: 'Eventos Críticos',
    description: 'Eventos de segurança nas últimas 24 horas',
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Análise detalhada do nível de risco da sua empresa
            </p>
          </div>
        </div>
        <Button 
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-8">
              ...
              <div className="text-center md:text-left">
                ...
                <p className="text-muted-foreground max-w-md">
                  {score >= 80 
                    ? "Sua empresa está bem protegida. Continue monitorando para manter esse nível."
                    : score >= 60
            ...
            <CardContent>
              {activeBreakdown.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-green-600 font-medium">Nenhuma penalidade ativa!</p>
                  <p className="text-sm text-muted-foreground mt-1">
                    Todos os fatores de segurança estão em ordem.
                  </p>
                </div>
              ) : (
                  ...
                  {activeBreakdown.map(([key, value]) => {
                    ...
                    return (
                      <Link
                        key={key}
                        to={config.link}
                        className="flex items-center justify-between p-3 rounded-lg bg-red-500/5 hover:bg-red-500/10 border border-red-500/20 transition-colors"
                      >
            ...
            <CardContent>
              {chartData.length < 2 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    O histórico será exibido após mais cálculos.
                  </p>
                </div>
              ) : (
                          ...
                          return (
                            <div className="bg-background border border-border rounded-lg p-2 shadow-lg">
                              <p className="text-sm font-medium">
                                Score: <span className={getScoreColor(payload[0].value as number)}>
                                  {payload[0].value}
                                </span>
                              </p>
                            </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {activeBreakdown
                  .sort((a, b) => a[1] - b[1]) // Sort by penalty (most negative first)
                  .slice(0, 3)
                  .map(([key, value]) => {
                    const config = BREAKDOWN_CONFIG[key as keyof RiskBreakdown];
                    if (!config) return null;
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">
                              Resolver problemas de {config.label.toLowerCase()}
                            </p>
                            <p className="text-xs text-muted-foreground">
                              Pode melhorar seu score em até {Math.abs(value)} pontos
                            </p>
                          </div>
                        </div>
