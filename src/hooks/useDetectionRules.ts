  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('id, tenant_id, rule_name, description, severity, event_type, is_enabled, mitre_technique_id, mitre_tactic, mitre_technique_name, confidence_base, tags, mode, risk_score, false_positive_count, true_positive_count, last_triggered_at, created_at, updated_at')
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('technique_id, technique_name, tactic, description, sub_technique_of, platforms, url')
        .order('technique_id');
      if (error) throw error;
      return data || [];
    },
