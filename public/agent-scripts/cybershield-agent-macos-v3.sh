...
# Version: v3.10.15-WEB-ACTIVITY-ENHANCED (Browser History: Safari + Chrome + Firefox)
...
system_metrics_json() {
  ...
  # RAM usada (%) - CORRIGIDO v3.6.0 - BUG CRITICO
  # macOS: usar vm_stat para calcular uso real
...
execute_job() {
  ...
  case "$job_type" in
    ...
    collect_web_activity)  # NOVO v3.10.15
      ...
      local safari_history="$HOME/Library/Safari/History.db"
      ...
      if [[ -f "$safari_history" ]] && command -v sqlite3 >/dev/null 2>&1; then
        local safari_temp="/tmp/safari_history_copy_$$.db"
...
main() {
  ...
  log "INFO" "============================================"
  log "INFO" "Iniciando CyberShield Agent - macOS v$AGENT_VERSION"
