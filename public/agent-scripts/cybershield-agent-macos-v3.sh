...
hmac_sign() {
  local message="$1"
  # macOS também usa openssl com hexkey
  printf '%s' "$message" \
...
secure_request() {
  ...
  while true; do
    ...
    # macOS date não tem %3N → usar segundos * 1000
...
system_metrics_json() {
  ...
  # CPU load médio (1 minuto) - usar "uptime"
  cpu_load="$(uptime | awk -F'load averages:' '{print $2}' 2>/dev/null | awk '{print $1}' | tr -d ',')"
  cpu_load="${cpu_load:-0}"

  # RAM usada (%)
  # macOS não tem free, usar memory_pressure é mais complexo; deixar 0 ou implementar depois
  ram_used="0"
