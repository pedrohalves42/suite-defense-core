...
2. Preencha com suas credenciais (obtidas do Supabase):
   ```bash
   SUPABASE_PUBLISHABLE_KEY=seu-valor-aqui
   SUPABASE_URL=https://seu-projeto.supabase.co
   ```

3. **IMPORTANTE**: Nunca versione `.env` ou `.env.local`
   - Estes arquivos estão automaticamente ignorados no `.gitignore`
   - Se você acidentalmente fizer commit, informe imediatamente

### Para Testes E2E

```bash
cp .env.test.example .env.test.local
# Preencha com credenciais de teste
```

### GitHub Actions & CI/CD

As credenciais são armazenadas como **Secrets** no repositório.
Não são necessárias variáveis de ambiente locais para CI/CD.

Ver: **Settings → Secrets and variables → Actions**
