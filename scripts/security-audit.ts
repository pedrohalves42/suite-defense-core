...
const EXCLUDED_DIRS = ["node_modules", ".git", "dev-dist", "playwright-report", "dist", "docs", "e2e", "tests", "__tests__", "supabase/migrations_archived", "public/agent-scripts", "supabase/functions/_shared/agent-scripts", "scripts"];
const EXCLUDED_FILES = ["package-lock.json", "bun.lockb", "src/integrations/supabase/types.ts", "database.types.ts", "vite.config.ts", ".env.example", ".env.test.example", ".env", "ApiKeys.tsx", "integracoes.ts", "constants.ts", "security-audit.ts", "agent-auth.ts", "serve-agent.ts", "index.test.ts", "index.ts", "EvidenceBundleExport.tsx"];
