...
# used in the codebase. These would cause state machine violations.
#
# Gate #3: Status proibidos no código
# - 'pending' (replaced by 'queued' in v3)
# - 'done' (never valid, use 'completed')
# ============================================================
...
# ============================================================
# Check for status: 'pending' in edge functions
# ============================================================
echo "Checking for forbidden 'status: pending' patterns..."

if grep -rn "status:\s*['\"]pending['\"]" supabase/functions --include="*.ts" 2>/dev/null; then
  echo ""
  echo "❌ FAIL: Found 'status: pending' in edge functions!"
  echo "   Use 'status: queued' instead (ADR-037)"
  echo ""
  ERRORS=$((ERRORS + 1))
else
  echo "✅ No 'status: pending' found in edge functions"
fi
...
if grep -rn "status:\s*['\"]pending['\"]" src --include="*.ts" --include="*.tsx" 2>/dev/null | grep -v "node_modules" | grep -v ".test."; then
  ...
  echo ""
  # Don't fail for client code, just warn
fi
