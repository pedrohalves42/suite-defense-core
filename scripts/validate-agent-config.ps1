...
<#
.SYNOPSIS
    Valida a configuração do agente CyberShield
.DESCRIPTION
    Verifica se o agente está configurado corretamente e pode autenticar
.PARAMETER AgentName
...
Write-Host "║   VALIDAÇÃO DE CONFIGURAÇÃO DO AGENTE      ║" -ForegroundColor Cyan
Write-Host "╚════════════════════════════════════════════╝`n" -ForegroundColor Cyan

# 1. Verificar se o script existe
Write-Host "1️⃣  Verificando arquivo do script..." -ForegroundColor Yellow
...
Write-Host "`n2️⃣  Verificando Scheduled Task..." -ForegroundColor Yellow
...
if ($task) {
    ...
    Write-Host "   ✓ Última execução: $($taskInfo.LastRunTime)" -ForegroundColor Green
    Write-Host "   ✓ Último resultado: $($taskInfo.LastTaskResult)" -ForegroundColor Green
    ...
    if ($args -match '-AgentToken\s+"?([a-f0-9\-]+)"?') {
        ...
        Write-Host "   ✗ AgentToken NÃO encontrado nos argumentos" -ForegroundColor Red
        $taskToken = $null
    }
    ...
    if ($args -match '-HmacSecret\s+"?([a-f0-9]+)"?') {
        ...
        Write-Host "   ✗ HmacSecret NÃO encontrado nos argumentos" -ForegroundColor Red
        $taskSecret = $null
    }
    ...
    if ($args -match '-AgentName\s+"?([^"]+)"?') {
        ...
        if ($taskAgentName -ne $AgentName) {
            Write-Host "   ⚠ ATENÇÃO: Nome na task ($taskAgentName) diferente do esperado ($AgentName)" -ForegroundColor Yellow
        }
    } else {
        Write-Host "   ✗ AgentName NÃO encontrado nos argumentos" -ForegroundColor Red
    }
} else {
    Write-Host "   ✗ Task NÃO encontrada: $taskName" -ForegroundColor Red
    Write-Host "   Execute recreate-agent-task.ps1 para criar" -ForegroundColor Red
    exit 1
}

# 3. Verificar conteúdo do script
Write-Host "`n3️⃣  Verificando conteúdo do script..." -ForegroundColor Yellow
...
if ($scriptContent -match 'param\s*\([\s\S]*?\[string\]\$AgentToken\s*=\s*"([^"]+)"') {
    ...
    Write-Host "   ⚠ AgentToken NÃO encontrado no param() do script" -ForegroundColor Yellow
    $scriptToken = $null
}
...
if ($scriptContent -match 'param\s*\([\s\S]*?\[string\]\$HmacSecret\s*=\s*"([^"]+)"') {
    ...
    Write-Host "   ⚠ HmacSecret NÃO encontrado no param() do script" -ForegroundColor Yellow
    $scriptSecret = $null
}

# 4. Comparar credenciais
Write-Host "`n4️⃣  Comparando credenciais..." -ForegroundColor Yellow
...
Write-Host "`n5️⃣  Verificando logs recentes..." -ForegroundColor Yellow
...
if (Test-Path $logPath) {
    ...
    if ($recentLogs) {
        Write-Host "   Últimas entradas relevantes:" -ForegroundColor Cyan
        ...
        if ($last401) {
            Write-Host "`n   ⚠ Último erro 401 detectado:" -ForegroundColor Yellow
        ...
        if ($lastSuccess) {
            Write-Host "`n   ✓ Última autenticação bem-sucedida:" -ForegroundColor Green
            Write-Host "   $lastSuccess" -ForegroundColor Green
        }
    } else {
        Write-Host "   ℹ Nenhuma entrada de autenticação encontrada nos logs recentes" -ForegroundColor Gray
    }
} else {
    Write-Host "   ⚠ Arquivo de log não encontrado: $logPath" -ForegroundColor Yellow
}

# 6. Verificar processos em execução
Write-Host "`n6️⃣  Verificando processos em execução..." -ForegroundColor Yellow
...
if ($processes) {
    ...
    Write-Host "   ⚠ Nenhum processo do agente em execução" -ForegroundColor Yellow
    Write-Host "     Execute: Start-ScheduledTask -TaskName $taskName" -ForegroundColor Cyan
}

# 7. Verificar conectividade
Write-Host "`n7️⃣  Verificando conectividade..." -ForegroundColor Yellow
...
# 8. Verificar sincronização de relógio
Write-Host "`n8️⃣  Verificando sincronização de relógio..." -ForegroundColor Yellow
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "   ⚠ Não foi possível verificar sincronização (w32tm não disponível)" -ForegroundColor Yellow
    }
} catch {
    Write-Host "   ⚠ Erro ao verificar sincronização: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
if (-not $processes) { $issues += "Agente não está em execução" }

if ($issues.Count -eq 0) {
    Write-Host "`n✅ CONFIGURAÇÃO OK - Nenhum problema detectado" -ForegroundColor Green
    ...
    Write-Host "`nRecomendação: Execute recreate-agent-task.ps1 com as credenciais corretas" -ForegroundColor Yellow
}
