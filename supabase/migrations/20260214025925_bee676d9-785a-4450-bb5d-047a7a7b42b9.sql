...
BEGIN
  ...
  WITH expired AS (
    UPDATE jobs
    SET status = 'failed', completed_at = now(), result = '{"error": "EXPIRED_TTL", "source": "maintenance"}'::jsonb
  ...
  -- 8. HMAC CLEANUP - FIX: use "used_at" (correct column name, not "created_at")
  WITH del3 AS (
...
-- =============================================================================
-- ZERO-GAP FIX #2: Reset consecutive_failures for fixed crons
-- =============================================================================
