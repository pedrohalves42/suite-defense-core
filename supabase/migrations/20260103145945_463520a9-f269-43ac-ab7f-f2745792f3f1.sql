...
BEGIN
  ...
  -- AI ACTIONS METRICS - FIXED: use human_reviewed = true instead of executed_by IS NOT NULL
  SELECT 
    COUNT(*),
    COUNT(*) FILTER (WHERE status = 'approved'),
    COUNT(*) FILTER (WHERE status = 'rejected'),
    COUNT(*) FILTER (WHERE status = 'pending'),
    COUNT(*) FILTER (WHERE human_reviewed = true),  -- FIX: was "executed_by IS NOT NULL"
  ...
  v_result := jsonb_build_object(
    ...
    'ai_actions', jsonb_build_object(
      ...
      'shadow_validation_rate', CASE WHEN v_ai_actions_total > 0 
        ...
        ELSE 0 END,
      'human_review_rate', CASE WHEN v_ai_actions_total > 0 
        THEN ROUND((v_ai_actions_human_reviewed::numeric / v_ai_actions_total::numeric) * 100, 2)
        ELSE 0 END
    ),
