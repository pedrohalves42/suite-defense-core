...
-- Fase 2: Função para sincronizar agent_state
...
BEGIN
  WITH updated AS (
    UPDATE agents SET 
      agent_state = 'offline',
      agent_state_reason = 'Sem heartbeat há mais de 24 horas (sync automático)',
...
BEGIN
  ...
  IF NEW.memory_usage_percent IS NOT NULL AND NEW.memory_usage_percent < 90 THEN
    UPDATE system_alerts SET resolved = true, resolved_at = NOW(), 
      resolution_notes = format('Auto-resolvido: Memória normalizada para %.1f%%', NEW.memory_usage_percent)
    WHERE (details->>'agent_id')::uuid = NEW.agent_id
