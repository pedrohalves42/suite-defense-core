...
-- Isso garante que quando um agent é deletado, seus jobs são removidos automaticamente
...
-- 2. Adicionar coluna key_hash em enrollment_keys para segurança
-- O key original será mantido temporariamente mas marcado para deprecação
ALTER TABLE public.enrollment_keys
ADD COLUMN IF NOT EXISTS key_hash TEXT;

-- 3. Criar índice para busca por key_hash
CREATE INDEX IF NOT EXISTS idx_enrollment_keys_key_hash 
ON public.enrollment_keys(key_hash) WHERE key_hash IS NOT NULL;

-- 4. Função para gerar hash de enrollment key
...
-- 6. Índice para acelerar cleanup de jobs por status e data
CREATE INDEX IF NOT EXISTS idx_jobs_status_created 
ON public.jobs(status, created_at) 
WHERE status IN ('pending', 'queued', 'delivered');

-- 7. Índice para acelerar queries de métricas por agent e tempo
...
-- 8. Índice para jobs por agent (acelera CASCADE e queries)
