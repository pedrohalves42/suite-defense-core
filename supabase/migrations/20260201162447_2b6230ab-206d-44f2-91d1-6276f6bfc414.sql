...
-- (A tabela já tem tenant_id, usaremos hash do script para criar cache key)
ALTER TABLE agent_builds
ADD COLUMN IF NOT EXISTS script_hash TEXT;

-- 2. Adicionar coluna cache_key computada para lookup rápido
ALTER TABLE agent_builds
...
-- 3. Índice para lookup rápido de builds cacheados
...
-- 4. Índice para buscar builds recentes completados por tenant
...
-- 5. Habilitar Realtime na tabela agent_builds para notificações push
ALTER PUBLICATION supabase_realtime ADD TABLE agent_builds;

-- 6. Comentário de documentação
COMMENT ON COLUMN agent_builds.cache_key IS 'Cache key computado: md5(tenant_id + script_hash + version). Usado para evitar rebuilds desnecessários.';
COMMENT ON COLUMN agent_builds.script_hash IS 'SHA256 do script do agente usado no build. Permite cache quando script não muda.';