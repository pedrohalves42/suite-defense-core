-- Restore comprehensive get_audit_raw_metrics function with corrected table/column references
-- Fixes: simplified version missing critical metrics for audit scoring
...
DECLARE
  ...
  -- Alert metrics (using system_alerts)
...
BEGIN
  ...
  INTO v_alerts_total, v_alerts_critical, v_alerts_high, v_alerts_medium, v_alerts_low, v_alerts_unresolved
  ...
  -- Policies with group assignments
  ...
  INTO v_ai_actions_total, v_ai_actions_approved, v_ai_actions_rejected, v_ai_actions_pending, v_ai_actions_human_reviewed
  ...
  -- =====================
  -- GOVERNANCE METRICS (failed_jobs_dlq)
  -- =====================
  SELECT 
    COUNT(*),
  ...
  SELECT 
    COUNT(*),
    COUNT(*) FILTER (WHERE last_execution_index > 0)
  ...
  -- RECENT ACTIVITY (24h)
  -- =====================
  SELECT COUNT(*) INTO v_alerts_24h
  FROM public.system_alerts
  WHERE tenant_id = p_tenant_id AND created_at > NOW() - INTERVAL '24 hours';
  
  SELECT COUNT(*) INTO v_ai_actions_24h
  FROM public.ai_actions
  WHERE tenant_id = p_tenant_id AND created_at > NOW() - INTERVAL '24 hours';
  
  SELECT COUNT(*) INTO v_agents_updated_24h
  FROM public.agents
