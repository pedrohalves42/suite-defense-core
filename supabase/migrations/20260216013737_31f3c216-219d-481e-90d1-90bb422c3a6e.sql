...
-- Recreate with agent_id filter fix
CREATE OR REPLACE FUNCTION claim_jobs_for_agent(p_agent_id uuid, p_limit int DEFAULT 3)
RETURNS TABLE(
  job_id uuid,
  job_type text,
  payload jsonb,
    ...
    WITH claimed AS (
      ...
      WHERE jobs.id IN (
        ...
        WHERE j.status IN ('queued', 'pending')
          ...
          AND j.agent_id = p_agent_id  -- FIX: Only claim jobs assigned to THIS agent
          AND j.approved = true
...
-- Maintain security: revoke from public/anon
REVOKE ALL ON FUNCTION claim_jobs_for_agent(uuid, int) FROM public, anon;
GRANT EXECUTE ON FUNCTION claim_jobs_for_agent(uuid, int) TO authenticated, service_role;
