...
BEGIN
  ...
  -- 2. VERSION: Versão desatualizada
        ...
        RETURN QUERY SELECT
          ...
          'Versão ' || v_agent.agent_version || ' desatualizada (atual: ' || v_latest_version || ')' ||
            CASE WHEN v_agent.force_update_version IS NOT NULL 
              THEN ' — atualização forçada pendente' ELSE '' END,
          jsonb_build_object(
  ...
  -- 4. JOBS: Jobs falhados recentes (últimas 24h, agrupados por tipo)
  RETURN QUERY
  SELECT
    'failed_job'::text,
    CASE WHEN j_grouped.fail_count > 3 THEN 'high' ELSE 'medium' END,
    j_grouped.job_type || ' falhou ' || j_grouped.fail_count || 'x nas últimas 24h' ||
  ...
  -- 5. SECURITY: Eventos de segurança críticos recentes (últimas 48h, deduplicados)
  RETURN QUERY
  SELECT DISTINCT ON (e.event_type, e.event_data->>'component', e.event_data->>'root_cause')
    ...
    CASE e.event_type
      ...
      WHEN 'service_stopped' THEN
        'Serviço ' || COALESCE(e.event_data->>'service_name', 'desconhecido') || ' parado'
      ELSE
