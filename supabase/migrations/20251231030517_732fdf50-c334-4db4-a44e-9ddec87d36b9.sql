...
VALUES 
  ('isolate_agent', 'Isola um agente da rede para contenção de ameaças', false, true, 'high', 5),
  ('revoke_token', 'Revoga todos os tokens ativos de um agente', false, true, 'high', 10),
  ('disable_user', 'Registra solicitação para desabilitar um usuário', false, true, 'high', 5),
  ('block_ip', 'Bloqueia um endereço IP específico', false, true, 'high', 10),
  ('include_firewall_rule', 'Adiciona uma regra de firewall no agente', true, true, 'medium', 20),
  ('restart_service', 'Reinicia um serviço específico no agente', true, true, 'medium', 15),
