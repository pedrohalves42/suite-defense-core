...
-- FIX: Make claim_jobs_for_agent resilient to execution_chain desync
-- Root cause: agent_execution_chain.last_execution_index can become stale if
-- a previous transaction inserted into job_executions but the chain update failed,
-- or if concurrent heartbeats created executions outside this RPC.
-- Solution: Read the actual max execution_index from job_executions as the source of truth,
-- not just from agent_execution_chain. This makes the RPC self-healing.
...
BEGIN
  ...
  IF v_actual_max_index IS NOT NULL AND v_actual_max_index > COALESCE(v_chain.last_execution_index, 0) THEN
    -- Get the hash for the actual max index
