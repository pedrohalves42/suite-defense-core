...
AS $$
DECLARE
  r1 jsonb := '{"skipped": true}'::jsonb;
  r2 jsonb := '{"skipped": true}'::jsonb;
  r3 jsonb := '{"skipped": true}'::jsonb;
...
BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    r1 := jsonb_build_object('error', SQLERRM);
  END;
  
  BEGIN
    SELECT cleanup_jobs_for_offline_agents() INTO r2;
  EXCEPTION WHEN OTHERS THEN
    r2 := jsonb_build_object('error', SQLERRM);
  END;
  
  BEGIN
    SELECT resolve_stale_dlq_entries() INTO r3;
  EXCEPTION WHEN OTHERS THEN
    r3 := jsonb_build_object('error', SQLERRM);
  END;
  
  BEGIN
    SELECT auto_resolve_stale_tasks() INTO r4;
  EXCEPTION WHEN OTHERS THEN
    r4 := jsonb_build_object('error', SQLERRM);
  END;
