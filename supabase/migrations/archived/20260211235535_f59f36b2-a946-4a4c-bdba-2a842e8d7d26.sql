...
-- =============================================================================
-- FIX-002: Fix execution chain race condition causing duplicate key errors
-- The claim_jobs_for_agent function reads chain state without locking,
-- allowing concurrent claims to generate the same execution_index.
-- Fix: Lock the chain row with FOR UPDATE before reading.
-- =============================================================================
...
BEGIN
  ...
  -- FIX: Lock chain row ONCE before processing all jobs to prevent race condition
  ...
  LOOP
    ...
    -- Calculate new execution index from running state (not re-reading from DB)
