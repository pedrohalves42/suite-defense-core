...
-- ============================================================================
-- Optimized maintenance RPC: replaces 3+ edge-function queries with 1 DB call
-- ============================================================================
...
BEGIN
  -- 1) Expire jobs past TTL (single UPDATE with CTE)
  WITH expired AS (
    UPDATE jobs
    SET status = 'timeout', updated_at = v_now
    WHERE id IN (
      SELECT id FROM jobs
  ...
  -- 2) Archive old executions (>30 days, single UPDATE with CTE)
  WITH archived AS (
  ...
  -- 3) Clean stale force_update flags pointing to non-existent releases (single UPDATE with subquery)
  WITH stale AS (
    UPDATE agents a
...
-- Restrict access: only service_role
...
-- Add index to speed up the expires_at scan (partial index on active statuses)
CREATE INDEX IF NOT EXISTS idx_jobs_expires_at_active 
ON public.jobs (expires_at) 
WHERE status IN ('pending', 'queued', 'delivered', 'running');

-- Add index to speed up archive scan
CREATE INDEX IF NOT EXISTS idx_job_executions_archive_candidates
