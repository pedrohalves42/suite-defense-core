...
BEGIN
  ...
  IF v_detection_rules_count > 0 THEN
    INSERT INTO soc2_evidence (tenant_id, control_id, evidence_type, reference, description, metadata, status, valid_from)
    VALUES (p_tenant_id, 'CC1.2', 'configuration', 'detection_rules',
      v_detection_rules_count || ' regra(s) de detecção configurada(s)',
      jsonb_build_object('count', v_detection_rules_count),
      'active', NOW());
    v_count := v_count + 1;
  END IF;
  ...
  IF v_detection_rules_count > 0 THEN
    INSERT INTO soc2_evidence (tenant_id, control_id, evidence_type, reference, description, metadata, status, valid_from)
    VALUES (p_tenant_id, 'CC3.1', 'configuration', 'detection_rules',
      v_detection_rules_count || ' regra(s) de monitoramento de risco',
  ...
  IF v_enrollment_total > 0 THEN
    ...
    VALUES (p_tenant_id, 'CC6.3', 'configuration', 'enrollment_keys',
      v_enrollment_active || '/' || v_enrollment_total || ' chave(s) de enrollment',
  ...
  IF v_detection_rules_count > 0 OR v_agents_total > 0 THEN
    INSERT INTO soc2_evidence (tenant_id, control_id, evidence_type, reference, description, metadata, status, valid_from)
    VALUES (p_tenant_id, 'CC7.2', 'configuration', 'detection_rules + agents',
