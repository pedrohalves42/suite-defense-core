/**
 * Approve via token handler — one-click approval via secure token link.
 */
...
interface ApprovalResult {
  ...
  playbook_name?: string; execution_id?: string; error?: string;
}
...
export async function handleApproveViaToken(
  ...
  const rateLimitResult = await checkRateLimit(supabase, clientIp, 'approve-via-token', { maxRequests: 10, windowMinutes: 15, blockMinutes: 60 });
  if (!rateLimitResult.allowed) {
    return generateHtmlResponse({ success: false, message: `Muitas tentativas. Tente novamente apos ${rateLimitResult.resetAt?.toLocaleString('pt-BR')}`, error: 'RATE_LIMITED' });
  }
  ...
  if (req.method === 'GET') {
    ...
    const parseResult = ApprovalTokenSchema.safeParse(payload);
    token = parseResult.success ? parseResult.data.token : null;
  }

  if (!token) return generateHtmlResponse({ success: false, message: 'Token de aprovacao nao fornecido', error: 'MISSING_TOKEN' });
  ...
  const { data: approvalRequest, error: findError } = await supabase
    ...
    .eq('approval_token', token).single();

  if (findError || !approvalRequest) return generateHtmlResponse({ success: false, message: 'Token de aprovacao invalido ou nao encontrado', error: 'INVALID_TOKEN' });
  if (approvalRequest.status !== 'pending') return generateHtmlResponse({ success: false, message: `Esta solicitacao ja foi processada (status: ${approvalRequest.status})`, error: 'ALREADY_PROCESSED' });

  const now = new Date();
  if (approvalRequest.approval_token_expires_at && new Date(approvalRequest.approval_token_expires_at) < now) return generateHtmlResponse({ success: false, message: 'O link de aprovacao expirou', error: 'TOKEN_EXPIRED' });
  if (approvalRequest.expires_at && new Date(approvalRequest.expires_at) < now) return generateHtmlResponse({ success: false, message: 'A solicitacao de aprovacao expirou', error: 'REQUEST_EXPIRED' });

  const actionPayload = approvalRequest.action_payload as Record<string, unknown>;
  const playbookName = (actionPayload?.playbook_name as string) || 'Playbook';
  const executionId = (actionPayload?.execution_id as string) || approvalRequest.playbook_execution_id;
  ...
  if (updateError) return generateHtmlResponse({ success: false, message: 'Erro ao processar aprovacao', error: 'UPDATE_FAILED' });

  await supabase.from('audit_logs').insert({
    tenant_id: approvalRequest.tenant_id, action: 'approve_via_token',
    resource_type: 'approval_request', resource_id: approvalRequest.id, success: true,
    details: { playbook_name: playbookName, execution_id: executionId, approved_via: 'one_click_link', client_ip: clientIp, user_agent: userAgent, request_id: requestId, processing_time_ms: Date.now() - startTime },
  });

  await supabase.from('risk_decision_log').insert({
    tenant_id: approvalRequest.tenant_id, playbook_execution_id: executionId,
    event_type: 'one_click_approval', playbook_id: actionPayload?.playbook_id as string,
  ...
  if (executionId) {
    await supabase.from('playbook_executions').update({ status: 'in_progress', auto_executed: false, triggered_by: 'one_click_approval' }).eq('id', executionId);
  }

  return generateHtmlResponse({ success: true, message: 'Aprovacao realizada com sucesso!', playbook_name: playbookName, execution_id: executionId });
}
