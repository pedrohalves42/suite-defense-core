/**
 * sync-router — DEPRECATED: Thin proxy to ops-gateway
 * Use ops-gateway with "sync:*" namespace instead.
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[sync-router][${requestId}] DEPRECATED proxy → ops-gateway sync:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/ops-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `sync:${action}`, payload: payload || {} }),
    timeoutMs: 45000,
  });
