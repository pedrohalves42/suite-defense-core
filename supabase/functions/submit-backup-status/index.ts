 ...
 * 
 * Accepts backup status for:
 * - Windows Backup / File History
 * - VSS Shadow Copies
 * - Third-party tools (Veeam, Acronis, Cobian)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { agent_id, backups } = body as { agent_id: string; backups: BackupEntry[] };

    if (!agent_id || !Array.isArray(backups)) {
      return new Response(JSON.stringify({ error: 'Missing agent_id or backups array' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    // Validate agent exists and get tenant_id
    ...
    for (const backup of backups) {
      ...
      // Create alert for critical/warning backup status
