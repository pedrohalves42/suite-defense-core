/**
 * evaluate-playbook-triggers - Modularized
...
Deno.serve(async (req) => {
  if (req.method === 'OPTIONS') return new Response(null, { headers: buildCorsHeaders(req.headers.get('origin')) });
  ...
  try {
    ...
    if (!isInternalCall) {
      if (!authHeader) {
        logger.error('[SECURITY] evaluate-playbook-triggers called without auth');
      ...
      if (authError || !user) return new Response(JSON.stringify({ error: 'Invalid token' }), { status: 401, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });

      const body: TriggerEvent = await req.json();
      const { data: userRole } = await supabase.from('user_roles').select('tenant_id, role').eq('user_id', user.id).eq('tenant_id', body.tenant_id).maybeSingle();
      if (!userRole) return new Response(JSON.stringify({ error: 'Access denied' }), { status: 403, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
      if (!['admin', 'super_admin'].includes(userRole.role)) return new Response(JSON.stringify({ error: 'Forbidden: Only admins can trigger playbooks' }), { status: 403, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    }

    const body: TriggerEvent = isInternalCall ? await req.json() : await req.json().catch(() => ({ tenant_id: '', trigger_type: '' as TriggerEvent['trigger_type'] }));
    const { tenant_id, trigger_type, agent_id, context = {} } = body;

    if (!tenant_id || !trigger_type) return new Response(JSON.stringify({ error: 'tenant_id and trigger_type are required' }), { status: 400, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    ...
    if (!playbooks || playbooks.length === 0) return new Response(JSON.stringify({ triggered: false, reason: 'No active playbooks for this trigger type' }), { headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    ...
    if (hasRecentExec) return new Response(JSON.stringify({ triggered: false, reason: 'Cooldown active', cooldown_minutes: cooldownMinutes }), { headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });

    // Evaluate conditions
    if (!evaluateConditions(trigger_type, playbook.trigger_conditions || {}, context)) {
      return new Response(JSON.stringify({ triggered: false, reason: 'Trigger conditions not met', conditions: playbook.trigger_conditions, context }), { headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    }
