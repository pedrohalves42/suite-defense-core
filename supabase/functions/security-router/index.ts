/**
 * security-router — DEPRECATED: Thin proxy to api-gateway
 * Use api-gateway with "security:*" namespace instead.
...
const FORWARDED_HEADERS = ['Authorization', 'apikey', 'X-Internal-Secret', 'X-Agent-Token', 'X-HMAC-Signature', 'X-Timestamp', 'X-Nonce', 'x-cron-source'];
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[security-router][${requestId}] DEPRECATED proxy → api-gateway security:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/api-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `security:${action}`, payload: payload || {} }),
    timeoutMs: 30000,
  });
