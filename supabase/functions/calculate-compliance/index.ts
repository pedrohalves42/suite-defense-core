...
serveTenant(async (_req, ctx) => {
  ...
  const { count: totalAgents } = await supabase
    ...
    .eq('tenant_id', tenantId).not('status', 'in', '("archived","deleted")');

  const { count: criticalVulns } = await supabase
    .from('vuln_findings').select('*', { count: 'exact', head: true })
    .eq('tenant_id', tenantId).ilike('severity', 'critical');

  const { count: highVulns } = await supabase
    .from('vuln_findings').select('*', { count: 'exact', head: true })
    .eq('tenant_id', tenantId).ilike('severity', 'high');
  ...
  const { count: expiredCerts } = await supabase
    .from('agent_certificates').select('*', { count: 'exact', head: true })
    .eq('tenant_id', tenantId).lt('valid_until', now);
  ...
  const { count: blockedUsb } = await supabase
    .from('agent_usb_devices').select('*', { count: 'exact', head: true })
    .eq('tenant_id', tenantId).eq('is_blocked', true);
  ...
  const { count: unresolvedAlerts } = await supabase
    .from('system_alerts').select('*', { count: 'exact', head: true })
  ...
  const { count: recentEvents } = await supabase
    .from('domain_events').select('*', { count: 'exact', head: true })
    .eq('tenant_id', tenantId).gte('occurred_on', oneDayAgo);
  ...
  const overallScore = Math.round(categories.reduce((sum, c) => sum + c.score * c.weight, 0));
  const grade = gradeFromScore(overallScore);

  // Drift Detection
  const { data: previousSnapshot } = await supabase
    .from('compliance_snapshots').select('overall_score, grade, calculated_at')
    .eq('tenant_id', tenantId).order('calculated_at', { ascending: false }).limit(1).maybeSingle();
  ...
  // Persist Snapshot
  const { error: snapshotError } = await supabase.from('compliance_snapshots').insert({
    tenant_id: tenantId, overall_score: overallScore, grade,
    category_scores: categories, calculated_at: new Date().toISOString(),
  });
  ...
  if (hasDrift && trend === 'degrading' && Math.abs(scoreDiff) >= 10) {
    await supabase.from('system_alerts').insert({
      tenant_id: tenantId, alert_type: 'compliance_drift',
      ...
      resolved: false,
      metadata: {
        previous_score: previousSnapshot!.overall_score, current_score: overallScore,
        drift: scoreDiff, grade,
  ...
  await supabase.from('domain_events').insert({
    aggregate_id: tenantId, aggregate_type: 'compliance',
    event_type: 'compliance.score_calculated',
    payload: { overall_score: overallScore, grade, has_drift: hasDrift, trend, score_diff: scoreDiff },
    occurred_on: new Date().toISOString(), tenant_id: tenantId,
  });
  ...
  try {
    await supabase.rpc('log_scheduled_job_run', {
      p_job_key: 'calculate-compliance', p_success: true, p_duration_ms: durationMs,
      p_result: { overall_score: overallScore, grade, has_drift: hasDrift, trend },
      p_processed_count: categories.length, p_job_source: 'api',
    });
  ...
  return {
    success: true, overall_score: overallScore, grade, categories,
