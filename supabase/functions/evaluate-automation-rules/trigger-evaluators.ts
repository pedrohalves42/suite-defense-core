...
export async function evaluateMetricThreshold(
  supabase: any, rule: Record<string, unknown>, tenantId: string, agents: any[], latestMetrics: Map<string, Record<string, unknown>>
...
export async function evaluateProcessAnomaly(
  supabase: any, rule: Record<string, unknown>, tenantId: string, agents: any[]
): Promise<TriggerCandidate[]> {
  const conditions = rule.trigger_conditions as Record<string, unknown>;
  const candidates: TriggerCandidate[] = [];
