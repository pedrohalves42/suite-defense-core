/**
 * billing-router — DEPRECATED: Thin proxy to api-gateway
 * Use api-gateway with "billing:*" namespace instead.
...
const FORWARDED_HEADERS = ['Authorization', 'apikey', 'X-Internal-Secret', 'x-cron-source'];
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[billing-router][${requestId}] DEPRECATED proxy → api-gateway billing:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/api-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `billing:${action}`, payload: payload || {} }),
    timeoutMs: 30000,
  });
