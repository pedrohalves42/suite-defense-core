 ...
 * 
 * Auth: Deno.serve + assertInternalCaller (cron function)
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (systemMode === 'halt_jobs') {
      return new Response(JSON.stringify({ success: false, error: 'SYSTEM_HALTED', message: 'Kill switch is active.' }), { status: 503, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    }
    ...
    if (!pendingActions || pendingActions.length === 0) {
      ...
      return new Response(JSON.stringify({ success: true, message: 'No pending actions', actions_processed: 0 }), { headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
    }
    ...
    return new Response(JSON.stringify({ success: true, request_id: requestId, duration_ms: duration, ...result }), { headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
  } catch (error) {
    const duration = Date.now() - startTime;
    logger.error(`[${requestId}] Error after ${duration}ms:`, error);
    const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
    await supabase.rpc('log_scheduled_job_run', { p_job_key: 'auto-execute-ai-actions', p_success: false, p_duration_ms: duration, p_error: error instanceof Error ? error.message : 'Unknown error', p_processed_count: 0, p_job_source: 'cron' });
    return new Response(JSON.stringify({ success: false, error: error instanceof Error ? error.message : 'Internal server error', request_id: requestId }), { status: 500, headers: { ...buildCorsHeaders(origin), 'Content-Type': 'application/json' } });
  }
});
