/**
 * Check Agent Integrity - Migrated to assertInternalCaller
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: problematicAgents, error: queryError } = await supabase
      .from('agents')
      .select('id, agent_name, tenant_id, status, last_heartbeat, enrolled_at, hostname, os_type')
      .eq('status', 'active')
    ...
    for (const agent of problematicAgents || []) {
      ...
      if (!agent.last_heartbeat) {
        ...
        minutesSinceHeartbeat = Math.floor((Date.now() - new Date(agent.last_heartbeat).getTime()) / (1000 * 60));
        issueType = (minutesSinceHeartbeat > 30 && minutesSinceHeartbeat < 1440) ? 'removed_after_reboot' : 'stale_after_active';
      }
      ...
      if (existingAlert) {
        ...
        await supabase.from('persistent_failure_alerts').update({
          failure_count: failureCount, last_failure_at: new Date().toISOString(),
        ...
        await supabase.from('persistent_failure_alerts').insert({
          tenant_id: agent.tenant_id, agent_id: agent.id,
          alert_type: 'agent_integrity_failure', failure_count: 1,
          first_failure_at: new Date().toISOString(), last_failure_at: new Date().toISOString(),
          metadata: { hostname: agent.hostname, os_type: agent.os_type, issue_type: issueType },
        });
      }

      issues.push({
        agent_id: agent.id, agent_name: agent.agent_name, tenant_id: agent.tenant_id,
        issue_type: issueType, last_heartbeat: agent.last_heartbeat, enrolled_at: agent.enrolled_at,
        minutes_since_heartbeat: minutesSinceHeartbeat, failure_count: failureCount,
      });

      if (issueType === 'removed_after_reboot') {
        alertsToCreate.push({
          tenant_id: agent.tenant_id, agent_id: agent.id,
          alert_type: 'agent_integrity_failure', severity: 'high',
          message: `Computador "${agent.agent_name}" parou de responder apos possivel reinicio. Ultimo sinal ha ${minutesSinceHeartbeat} minutos.`,
          resolved: false,
          metadata: {
            issue_type: issueType, hostname: agent.hostname, os_type: agent.os_type,
    ...
    for (const immediateAlert of immediateAlertsToSend) {
      try {
        ...
        await supabase.functions.invoke('security-alert-dispatcher', {
          body: {
            type: 'agent_persistent_failure', severity: 'critical', immediate: true,
            tenant_id: a.agent.tenant_id, agent_id: a.agent.id,
            agent_name: a.agent.agent_name, failure_count: a.failureCount,
            minutes_since_heartbeat: a.minutesSinceHeartbeat,
            message: `CRITICO: Agente "${a.agent.agent_name}" com ${a.failureCount} falhas consecutivas.`,
          },
        });
    ...
    if (agentsToDeactivate.length > 0) {
      await supabase.from('agents').update({ status: 'inactive' }).in('id', agentsToDeactivate);
    }
    ...
    for (const issue of issues.filter(i => i.issue_type === 'removed_after_reboot' || i.issue_type === 'persistent_failure')) {
      await supabase.from('agent_events').insert({
        ...
        details: {
          issue_type: issue.issue_type, last_heartbeat: issue.last_heartbeat,
          minutes_since_heartbeat: issue.minutes_since_heartbeat, failure_count: issue.failure_count,
          detected_at: new Date().toISOString(), recommendation: 'Reinstall agent after adding Windows Defender exclusion',
        },
      });
    }
    ...
    try {
      await supabase.rpc('log_scheduled_job_run', {
        p_job_key: 'check-agent-integrity', p_success: true,
        p_duration_ms: durationMs, p_result: summary,
        p_processed_count: problematicAgents?.length || 0, p_job_source: 'cron',
      });
    } catch (logErr) { logger.warn(`[${requestId}] Failed to log job run:`, logErr); }
    ...
    try {
      ...
      await supabase.rpc('log_scheduled_job_run', {
        p_job_key: 'check-agent-integrity', p_success: false,
        p_duration_ms: durationMs, p_error: error instanceof Error ? error.message : 'Unknown error',
        p_result: null, p_processed_count: 0, p_job_source: 'cron',
      });
    } catch (logErr) { logger.warn(`[${requestId}] Failed to log error:`, logErr); }
