/**
 * admin-router — DEPRECATED: Thin proxy to api-gateway
 * Use api-gateway with "admin:*" namespace instead.
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[admin-router][${requestId}] DEPRECATED proxy → api-gateway admin:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/api-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `admin:${action}`, payload: payload || {} }),
    timeoutMs: 25000,
  });
