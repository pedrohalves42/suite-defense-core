/**
 * admin-router — Consolidated Admin & User Management Router
 * 
 * Routes: admin-create-user, list-all-users-admin, list-users,
 *   update-user-role, update-user-status, update-member-role, remove-member,
 *   set-active-tenant, get-admin-releases, api-tenant-features,
 *   api-tenant-info, api-tenant-stats, get-rate-limit-stats
 * 
 * Auth: JWT required (admin role), forwarded to sub-functions
 */
...
const VALID_ACTIONS = new Set([
  'create-user', 'list-all-users', 'list-users',
  'update-user-role', 'update-user-status', 'update-member-role',
  'remove-member', 'set-active-tenant', 'get-admin-releases',
  'tenant-features', 'tenant-info', 'tenant-stats', 'rate-limit-stats',
]);
...
const ACTION_TO_FUNCTION: Record<string, string> = {
  ...
  'set-active-tenant': 'set-active-tenant',
  'get-admin-releases': 'get-admin-releases',
  'tenant-features': 'api-tenant-features',
  'tenant-info': 'api-tenant-info',
  'tenant-stats': 'api-tenant-stats',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    logger.info(`[admin-router] ${action} completed in ${elapsed}ms (status: ${response.status})`);
