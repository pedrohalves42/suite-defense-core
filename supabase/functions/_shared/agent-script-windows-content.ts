...
try {
  ...
  console.log(`[agent-script-loader] Loaded Windows v5 script from file: ${_content.length} chars`);
} catch {
  // Expected in Deno Deploy - .ps1 not bundled
  // Edge functions will use DB content from agent_releases table
  _content = '';
  console.log('[agent-script-loader] Windows v5 script not available from file (expected in Deploy)');
}
