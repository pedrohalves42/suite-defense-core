...
export async function handleSystemMetrics(
  ...
  const primaryDisk = selectPrimaryDisk(
    metrics.disks, metrics.disk_total_gb, metrics.disk_used_gb,
    metrics.disk_free_gb, metrics.disk_usage_percent,
  );
  ...
  const { error: insertError } = await supabase.from('agent_system_metrics_partitioned').insert({
    ...
    memory_total_gb: metrics.memory_total_gb, memory_used_gb: metrics.memory_used_gb,
    memory_free_gb: metrics.memory_free_gb, memory_usage_percent: metrics.memory_usage_percent,
    disk_total_gb: primaryDisk.total_gb, disk_used_gb: primaryDisk.used_gb,
    disk_free_gb: primaryDisk.free_gb, disk_usage_percent: primaryDisk.usage_percent,
  ...
  const alertsGenerated = await generateAlerts(supabase, { id: agentId, agent_name: agentName, tenant_id: tenantId }, {
    ...
    memory_used_gb: metrics.memory_used_gb, memory_total_gb: metrics.memory_total_gb,
  ...
  return { success: true, alerts_generated: alertsGenerated, automation_triggered: 0, light_mode: lightModeConfig };
}
