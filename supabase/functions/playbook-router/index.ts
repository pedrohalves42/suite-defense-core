...
async function handleAutoTriageInsights(supabase: SB, requestId: string) {
  ...
  if (triagedCount > 0) {
    try { await supabase.from('audit_logs').insert({ action: 'auto_triage_insights', resource_type: 'ai_insight', resource_id: 'system_cron', details: { triaged_count: triagedCount, insight_ids: updated?.map(i => i.id) || [] }, success: true }); } catch { /* non-critical */ }
  }
