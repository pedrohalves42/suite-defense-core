...
Deno.serve(async (req) => {
  ...
  try {
    ...
    logger.info(`Fetching timeline for agent ${agent.agent_name}`);
    ...
    if (eventsError) {
      logger.error('Failed to fetch timeline events', eventsError);
      return new Response(
    ...
    logger.success(`Timeline fetched: ${events?.length || 0} events`);
    ...
    logger.error('Timeline fetch failed', error);
    return new Response(
