/**
 * process-playbook-trigger-logs — Migrated to serveInternal
 * 
...
serveInternal(async (_req, ctx) => {
  ...
  if (!pendingLogs || pendingLogs.length === 0) {
    ...
    return { success: true, processed: 0, message: 'No pending logs', duration_ms: Date.now() - startTime };
  }
  ...
  return { success: true, processed: results.success, failed: results.failed, expired: results.expired, total: pendingLogs.length, duration_ms: duration };
});
