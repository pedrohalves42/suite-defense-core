...
export async function handleSecurity(
  ...
  } catch (err) { logger.warn('[notification-router] audit log insert failed', err); }
...
async function sendSecurityEmail(
  ...
  try {
    ...
    if (expiresAt) try { expiresFormatted = new Date(expiresAt).toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }); } catch (err) { logger.warn('[notification-router] date format error', err); expiresFormatted = expiresAt; }
