/**
 * mitre-sync handler — inlined from standalone mitre-sync function
 */
...
export async function handleMitreSync(
  supabase: SB, requestId: string, payload: Record<string, unknown>,
): Promise<unknown> {
  const parsed = MitreSchema.safeParse(payload ?? {});
  if (!parsed.success) return { __status: 400, error: 'Invalid input', issues: parsed.error.flatten().fieldErrors };

  const action = parsed.data.action;

  if (action === 'sync') return await syncMitreRules(supabase);

  if (action === 'rules') {
    const { data, error } = await supabase.from('mitre_rules').select('id, technique_id, name, tactic, platform, is_active, last_synced_at').eq('is_active', true).order('technique_id');
    if (error) throw error;
    return { rules: data ?? [] };
  }

  if (action === 'version') {
    const { data } = await supabase.from('mitre_metadata').select('version, synced_at, total_rules, new_rules, updated_rules').order('synced_at', { ascending: false }).limit(1).maybeSingle();
    return { metadata: data ?? { version: 'unknown', synced_at: null } };
  }

  if (action === 'stale') {
    const { data } = await supabase.from('mitre_rules').select('technique_id, name, last_synced_at').eq('is_active', true).lt('last_synced_at', new Date(Date.now() - 30 * 86400000).toISOString()).order('last_synced_at');
    return { stale_rules: data ?? [] };
  }

  return { __status: 400, error: `Unknown action: ${action}` };
}

async function syncMitreRules(supabase: SB) {
  const t0 = Date.now();
  ...
  const mitreVersion = stix.objects?.find((o: Record<string, unknown>) => o.type === 'x-mitre-collection')?.x_mitre_version ?? stix.spec_version ?? 'unknown';
  ...
  for (let i = 0; i < techniques.length; i += BATCH) {
    ...
    const { data, error } = await supabase.from('mitre_rules').upsert(rows, { onConflict: 'technique_id', ignoreDuplicates: false }).select('technique_id');
    if (error) logger.error('[mitre-sync] Batch error:', error.message);
    else synced += data?.length ?? 0;
  }

  const durationMs = Date.now() - t0;
  await supabase.from('mitre_metadata').insert({ version: mitreVersion, synced_at: new Date().toISOString(), total_rules: techniques.length, new_rules: synced, updated_rules: 0, sync_duration_ms: durationMs });

  logger.info(`[mitre-sync] Done in ${durationMs}ms: version=${mitreVersion}, upserted=${synced}`);
