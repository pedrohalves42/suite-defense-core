/**
 * Generate Weekly Report - Migrated to serveInternal middleware
...
serveInternal(async (_req, ctx) => {
  ...
  for (const tenant of tenants || []) {
    ...
    try {
      ...
      logger.info(`[generate-weekly-report] Notification sent for ${tenant.name}`);
    } catch (emailError) {
      logger.error(`[generate-weekly-report] Notification error for ${tenant.name}:`, emailError);
    }
  }
