 ...
 * Extracted from ai-get-insights for direct dispatch.
 */
...
export async function handleGetInsights(
  ...
  if (!isInternal && userId) {
    const rateLimitResult = await checkRateLimit(supabase, userId, 'ai-get-insights', {
      maxRequests: 60, windowMinutes: 1, blockMinutes: 2,
    });
  ...
  return {
    ...
    statistics: {
      total: totalCount || 0, critical: criticalCount || 0, warning: warningCount || 0,
      info: infoCount || 0, acknowledged: acknowledgedCount || 0, pending: pendingCount || 0,
    },
