...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ============================================================
    // 1. VERIFICAR VIOLAÇÕES DE INTEGRIDADE (últimos 15 minutos)
    // ============================================================
    const { data: violations, error: violationsError } = await supabase
      .from('job_integrity_violations')
      .select('*')
      ...
      for (const [tenantId, tenantViolations] of violationsByTenant) {
        const { error: alertError } = await supabase
          ...
          .insert({
            ...
            data: {
              violations: tenantViolations.map(v => ({
                job_id: v.job_id,
                type: v.type,
                agent_id: v.agent_id,
                created_at: v.created_at
    ...
    return new Response(
      JSON.stringify({
        ...
        duration_ms: duration,
        violations_found: violations?.length || 0,
        alerts_created: violations && violations.length > 0 ? new Set(violations.map(v => v.tenant_id)).size : 0
      }),
