/**
 * agent-mgmt-router — Consolidated Agent Management Router (Phase 3)
 * Auth: Mixed (JWT, HMAC, Agent token), forwarded to sub-functions
 */
...
const ACTION_TO_FUNCTION: Record<string, string> = {
  'agent-snapshot': 'agent-snapshot', 'agent-version-management': 'agent-version-management',
  'check-agent-integrity': 'check-agent-integrity', 'check-agent-name-availability': 'check-agent-name-availability',
  'check-agent-updates': 'check-agent-updates', 'diagnose-agent': 'diagnose-agent',
  'diagnostics-agent-logs': 'diagnostics-agent-logs', 'enroll-agent': 'enroll-agent',
  'get-agent-config': 'get-agent-config', 'get-agent-dashboard-data': 'get-agent-dashboard-data',
  'get-agent-policy': 'get-agent-policy', 'get-agent-script-content': 'get-agent-script-content',
  'get-agent-timeline': 'get-agent-timeline', 'get-latest-agent-script': 'get-latest-agent-script',
  'promote-agent-v5': 'promote-agent-v5', 'recover-agent-credentials': 'recover-agent-credentials',
  'register-agent-key': 'register-agent-key', 'serve-agent-update': 'serve-agent-update',
  'setup-agent-script': 'setup-agent-script', 'token-rotate': 'token-rotate',
  'validate-hmac-signature': 'validate-hmac-signature', 'force-reinstall-fleet': 'force-reinstall-fleet',
  'create-reinstall-jobs': 'create-reinstall-jobs', 'get-reinstall-by-name': 'get-reinstall-by-name',
  'get-reinstall-preserve-script': 'get-reinstall-preserve-script', 'get-reinstall-script': 'get-reinstall-script',
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    logger.info(`[agent-mgmt-router] ${action} done in ${Date.now() - startedAt}ms (status: ${response.status})`);
