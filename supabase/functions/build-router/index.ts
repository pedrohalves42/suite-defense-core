/**
 * build-router — DEPRECATED: Thin proxy to api-gateway
 * Use api-gateway with "build:*" namespace instead.
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[build-router][${requestId}] DEPRECATED proxy → api-gateway build:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/api-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `build:${action}`, payload: payload || {} }),
    timeoutMs: 60000,
  });
