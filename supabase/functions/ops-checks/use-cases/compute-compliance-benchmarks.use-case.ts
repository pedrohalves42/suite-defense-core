// compute-compliance-benchmarks.use-case.ts
import { ICheckRepository } from '../../_shared/hexagonal/repositories/check.repository.ts';
import { logger } from '../../_shared/logger.ts';

export class ComputeComplianceBenchmarksUseCase {
  ...
  async execute(requestId: string) {
    ...
    await (this.checkRepository as any).supabase.from('compliance_benchmarks').upsert({
  ...
  private async calculateTenantComplianceScore(tenantId: string): Promise<{ overall: number; categories: Record<string, number> } | null> {
    try {
      const categories: Record<string, number> = {};
      const { data: agents } = await (this.checkRepository as any).supabase.from('agents').select('id, status').eq('tenant_id', tenantId);
      ...
      const { data: alerts } = await (this.checkRepository as any).supabase.from('system_alerts').select('id, acknowledged').eq('tenant_id', tenantId).limit(100);
      const totalAlerts = alerts?.length || 0;
      categories['alert_response'] = totalAlerts > 0 ? Math.round((alerts?.filter((a: any) => a.acknowledged).length || 0) / totalAlerts * 100) : 100;

      const { data: jobs } = await (this.checkRepository as any).supabase.from('jobs').select('id, status').eq('tenant_id', tenantId).limit(500);
      const totalJobs = jobs?.length || 0;
      categories['job_reliability'] = totalJobs > 0 ? Math.round((jobs?.filter((j: any) => j.status === 'completed').length || 0) / totalJobs * 100) : 0;

      const { count: evidenceCount } = await (this.checkRepository as any).supabase.from('agent_evidence_logs').select('id', { count: 'exact', head: true }).eq('tenant_id', tenantId);
      categories['evidence_coverage'] = Math.min(100, Math.round(((evidenceCount || 0) / 50) * 100));

      const { count: threatCount } = await (this.checkRepository as any).supabase.from('threat_indicators').select('id', { count: 'exact', head: true }).eq('is_active', true);
