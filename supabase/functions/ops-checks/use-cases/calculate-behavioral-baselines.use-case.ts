...
export class CalculateBehavioralBaselinesUseCase {
  ...
  async execute(requestId: string) {
    ...
    logger.info(`[${requestId}] CalculateBehavioralBaselinesUseCase: Starting baseline calculation...`);
    ...
    logger.info(`[${requestId}] CalculateBehavioralBaselinesUseCase: Processing ${agents.length} agents`);
    ...
    for (const agent of agents) {
      try {
        const { data: processData } = await (this.checkRepository as any).supabase
        ...
        for (const { type, values } of [{ type: 'cpu_usage', values: cpuValues }, { type: 'process_count', values: processCountValues }]) {
          ...
          const baselinePayload = {
            ...
            baseline_data: { sample_count: values.length, min: Math.min(...values), max: Math.max(...values), p50: this.percentile(values, 50), p90: this.percentile(values, 90), p99: this.percentile(values, 99) },
          ...
          const { error: upsertError } = await (this.checkRepository as any).supabase.from('agent_behavioral_baseline').upsert(baselinePayload, { onConflict: 'agent_id,baseline_type', ignoreDuplicates: false });

          if (upsertError) {
            const { data: existing } = await (this.checkRepository as any).supabase.from('agent_behavioral_baseline').select('id').eq('agent_id', agent.id).eq('baseline_type', type).maybeSingle();
            if (existing) { await (this.checkRepository as any).supabase.from('agent_behavioral_baseline').update(baselinePayload).eq('id', existing.id); }
            else { await (this.checkRepository as any).supabase.from('agent_behavioral_baseline').insert(baselinePayload); }
          }
        }
        processedCount++;
      } catch (agentError) {
        logger.error(`[${requestId}] CalculateBehavioralBaselinesUseCase: Error processing agent ${agent.agent_name}:`, String(agentError));
        errorCount++;
      }
    }
