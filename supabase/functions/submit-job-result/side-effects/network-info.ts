...
export async function processNetworkInfo(ctx: SubmitContext): Promise<void> {
  ...
  try {
    ...
    const networkRecord = {
      ...
      firewall_domain: (outputData.firewall_domain ?? outputData.FirewallDomain ?? null) as any,
      firewall_private: (outputData.firewall_private ?? outputData.FirewallPrivate ?? null) as any,
      firewall_public: (outputData.firewall_public ?? outputData.FirewallPublic ?? null) as any,
      open_ports: (outputData.open_ports || outputData.OpenPorts || []) as any,
      active_connections: ((outputData.active_connections || outputData.ActiveConnections || []) as any[]).slice(0, 100),
