 ...
 * Returns prioritized tips based on real data gaps.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[${requestId}] Analyzing security posture for tenant ${tenant_id}`);
...
Retorne um array JSON com objetos: { "title": string, "description": string, "severity": "info"|"warning"|"critical", "actionPath": string, "actionLabel": string }`
        ...
        if (aiResponse.ok) {
          ...
          console.error(`[${requestId}] AI gateway error: ${aiResponse.status}`);
        }
      } catch (aiErr) {
        console.error(`[${requestId}] AI generation failed:`, aiErr);
      }
    }
    ...
    console.log(`[${requestId}] Security advisor complete: ${aiTips.length} tips, maturity=${maturityLabel}`);
