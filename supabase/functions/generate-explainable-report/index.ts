/**
 * generate-explainable-report — Migrated to serveTenant
 * Generates explainable AI decision report for compliance and auditing.
 */
...
interface ExplainableReport {
  report_id: string; tenant_id: string; tenant_name: string;
  period: { start: string; end: string }; generated_at: string;
  executive_summary: {
    total_insights: number; total_decisions: number; auto_executed: number;
    manual_approved: number; effectiveness_rate: number; risk_categories: Record<string, number>;
  };
  decisions: DecisionEntry[];
  governance: { actions_within_policy: number; custom_policies_used: number; default_policies_used: number };
  evidence_hashes: Array<{ decision_id: string; hash: string }>;
}
...
function getPolicyApplied(_insightType: string, hasCustomPolicy: boolean, executionMode: string): string {
  return hasCustomPolicy
    ? `Politica personalizada do tenant: ${executionMode}`
    : `Politica padrao do sistema: ${executionMode}`;
}

serveTenant<ReportRequest>(async (_req, ctx) => {
  const { supabase, tenantId, userId, requestId, body } = ctx;
  const { period_start, period_end, format = 'json' } = body;

  if (!period_start || !period_end) {
    return new Response(JSON.stringify({ error: 'Missing required fields: period_start, period_end' }), { status: 400 });
  }
  ...
  const { data: customPolicies } = await supabase
    .from('tenant_action_policies').select('insight_type, execution_mode').eq('tenant_id', tenantId);
  ...
  if (format === 'html') {
    return new Response(generateHTMLReport(report), {
...
function generateHTMLReport(report: ExplainableReport): string {
  const decisionsRows = report.decisions.map(d => `
    <tr>
      ...
      <td><span class="badge badge-${d.effectiveness}">${d.effectiveness === 'resolved' ? 'Resolvido' : d.effectiveness === 'partial' ? 'Parcial' : d.effectiveness === 'failed' ? 'Falhou' : 'Pendente'}</span></td>
    </tr>
  `).join('');

  return `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Relatorio de Decisoes AI - ${report.tenant_name}</title>
