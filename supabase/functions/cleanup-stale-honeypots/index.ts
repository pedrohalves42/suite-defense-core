 ...
 * - Cleans up old honeypot_interactions (>90 days)
 */
...
serveInternal(async (_req, { supabase, requestId }) => {
  ...
  // 2. Cleanup old rate limit entries (>1 hour)
  const { data: cleanedRL } = await supabase.rpc('cleanup_honeypot_rate_limits', {
    p_older_than_minutes: 60,
  });
  results.rate_limits_cleaned = cleanedRL || 0;

  // 3. Cleanup old interactions (>90 days)
