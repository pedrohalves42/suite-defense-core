...
servePublic(async (req, ctx) => {
  ...
  try { await supabase.functions.invoke('ops-gateway', { body: { action: 'notify:webhook', payload: { type: 'new_contact', data: { name, email, company, endpoints } } }, headers: { 'X-Internal-Secret': Deno.env.get('INTERNAL_FUNCTION_SECRET') || '' } }); } catch (_) { /* non-critical */ }
