/**
 * check-router — DEPRECATED: Thin proxy to ops-gateway
 * Use ops-gateway with "check:*" namespace instead.
...
servePublic(async (req, ctx) => {
  ...
  logger.info(`[check-router][${requestId}] DEPRECATED proxy → ops-gateway check:${action}`);
  ...
  const response = await fetchWithTimeout(`${SUPABASE_URL}/functions/v1/ops-gateway`, {
    method: 'POST', headers,
    body: JSON.stringify({ action: `check:${action}`, payload: payload || {} }),
    timeoutMs: 30000,
  });
