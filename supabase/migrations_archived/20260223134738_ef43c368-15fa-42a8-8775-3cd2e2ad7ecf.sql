...
-- =====================================================
-- COST-OPT: Reduzir frequencia de pg_cron jobs
-- Objetivo: Reduzir ~70% das invocacoes de edge functions via cron
-- =====================================================
...
-- 13. run-rls-tests: 0 */6 ? 0 4 * * * (diario)
SELECT cron.alter_job(77, schedule := '0 4 * * *');

-- 14. Desativar jobs redundantes (ja cobertos por scheduled_jobs + invoke-scheduled-jobs)
-- ai-system-analyzer ja esta no scheduled_jobs
SELECT cron.alter_job(14, schedule := '0 4 * * *');

-- 15. generate-weekly-report: */30 (ABSURDO!) ? semanal
SELECT cron.alter_job(49, schedule := '0 6 * * 1');

-- 16. hmac-cleanup: */2h ? diario
...
-- 23. calculate-compliance: semanal OK, mantem
