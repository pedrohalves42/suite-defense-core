...
-- =============================================================================
-- FIX: Align V-101/V-102 logging with security_logs schema
-- Schema uses: attack_type, ip_address, endpoint, blocked (NOT event_type)
-- =============================================================================

-- V-101 FIX: cleanup_problematic_agent - Correct security_logs columns
...
BEGIN
  ...
  IF v_caller_tenant_id IS NULL AND NOT is_current_super_admin() THEN
    INSERT INTO security_logs (
      ...
      'rpc/cleanup_problematic_agent',
      true,
      jsonb_build_object('agent_id', p_agent_id, 'reason', 'No tenant context'),
      NULL,
      auth.uid()
    );
...
-- V-102 FIX: claim_jobs_for_agent - Correct security_logs columns
