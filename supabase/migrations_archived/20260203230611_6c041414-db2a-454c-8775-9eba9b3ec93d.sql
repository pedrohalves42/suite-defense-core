...
SELECT 
  ...
  -- INV-003: Isolamento Multi-Tenant
  ...
  -- INV-004: Secrets Protegidos (exclui views admin documentadas com is_current_super_admin)
  (
    SELECT COUNT(*) = 0
    FROM pg_views v
  ...
  (SELECT COUNT(*) FROM pg_views v WHERE v.schemaname = 'public' AND v.viewname IN ('agents_public', 'agents_safe', 'active_agents') AND POSITION('hmac' || '_secret' IN v.definition) = 0) AS inv004_safe_agent_views,
  
  -- INV-005: Auditoria Habilitada
