...
# HOTFIX-SKIP-FW-BOOT: Load persisted skip flag from HARDCODED C:\CyberShield path
try {
...
try {
    $skipFwPaths = @("C:\CyberShield\skip_firewall.flag", ("C:\CyberShield\skip_firewall.flag"))
        ...
        if ($result.Success) {
            ...
            if ($result.Content) {
                try {
                    ...
                    # ============================================
                    if ($(if ($response -and (Get-Member -InputObject $response -Name "force_update" -ErrorAction SilentlyContinue)) { $response.force_update } else { $false }) <# HOTFIX-FORCE-UPDATE #> -eq $true) {
                    ...
                    # ============================================
                    if ($(if ($response -is [hashtable] -and $response.ContainsKey("script_sha256")) { $response["script_sha256"] } elseif ($response -and (Get-Member -InputObject $response -Name "script_sha256" -ErrorAction SilentlyContinue)) { $response.script_sha256 } elseif ($response -and (Get-Member -InputObject $response -Name "sha256" -ErrorAction SilentlyContinue)) { $response.sha256 } else { $null }) <# HOTFIX-SAFE-SHA256 #>) {
                        try {
                            ...
                            Save-SignedHashCache -Hash $(if ($response -is [hashtable] -and $response.ContainsKey("script_sha256")) { $response["script_sha256"] } elseif ($response -and (Get-Member -InputObject $response -Name "script_sha256" -ErrorAction SilentlyContinue)) { $response.script_sha256 } elseif ($response -and (Get-Member -InputObject $response -Name "sha256" -ErrorAction SilentlyContinue)) { $response.sha256 } else { $null }) <# HOTFIX-SAFE-SHA256 #> -Signature $hashSig -Timestamp $hashTs
                        } catch {
    ...
    try {
        ...
        if ($disabledProfiles.Count -gt 0) {
            ...
            if ($Global:SkipFirewallRemediation -or (Test-Path "C:\CyberShield\skip_firewall.flag" -ErrorAction SilentlyContinue)) {
                Write-Log "[LOCAL-DETECT] Firewall disabled but SKIP active (external firewall). NO remediation." "INFO"
                Invoke-PushAlert -AlertType "firewall_disabled" -AlertMessage "Firewall desativado em $env:COMPUTERNAME (profiles: $($disabledProfiles -join ', ')). Remediacao pulada: firewall externo." -Severity "info" -Details @{ disabled_profiles = $disabledProfiles; skip_reason = "external_firewall"; auto_remediated = $false }
...
function Test-UsbDevices {
    try {
        ...
        if ($usbDrives -and @($usbDrives).Count -gt 0) <# HOTFIX-USB-COUNT #> {
            ...
            return @{ status = "detected"; count = @($usbDrives).Count <# HOTFIX-USB-COUNT #>; devices = $usbDrives | ForEach-Object { $_.Model } }
        }
...
function Invoke-LocalDetection {
    ...
    if ($results.antivirus -is [hashtable] -and $results.antivirus.status -eq "inactive") <# HOTFIX-TYPESAFE-STATUS #> { $results.threats_found++ }
    ...
    if ($results.usb -is [hashtable] -and $results.usb.status -eq "detected") <# HOTFIX-TYPESAFE-STATUS #> { $results.threats_found += $results.usb.count }
    
    $results.processes = @(Test-SuspiciousProcesses)[-1] <# PIPELINE-SAFE #>
    if ($results.processes -is [hashtable] -and $results.processes.status -eq "detected") <# HOTFIX-TYPESAFE-STATUS #> { $results.threats_found += $results.processes.count }
...
# ============================================
Write-Log "============================================" "INFO"
# HOTFIX-CNG-STARTUP-CLEANUP: Remove orphaned CNG containers to prevent key generation conflicts
try {
...
if ($Global:SecurityDegraded) {
    Write-Log "[STARTUP] Agent $($Global:AgentVersion) <# HOTFIX-VERSION-PREFIX #> starting in DEGRADED mode (SecurityDegraded=TRUE, only recovery jobs allowed)" "WARN"
} else {
    Set-AgentState -NewState "ENFORCING" -Reason "Normal operation"
    Write-Log "[STARTUP] Agent $($Global:AgentVersion) <# HOTFIX-VERSION-PREFIX #> fully operational in ENFORCING state" "SUCCESS"
}
...
try { Invoke-LocalDetection | Out-Null } catch { Write-Log "[LOCAL-DETECT] Non-fatal error: $($_.Exception.Message)" "WARN" } <# HOTFIX-LOCAL-DETECT-TRYCATCH #>
...
while ($true) {
    ...
    try {
        ...
        if ($taskHealth.checked -and $(if ($taskHealth -is [hashtable] -and $taskHealth.ContainsKey("repaired")) { $taskHealth["repaired"] } elseif ($taskHealth -and (Get-Member -InputObject $taskHealth -Name "repaired" -ErrorAction SilentlyContinue)) { $taskHealth.repaired } else { $false }) <# HOTFIX-SAFE-REPAIRED #>) {
